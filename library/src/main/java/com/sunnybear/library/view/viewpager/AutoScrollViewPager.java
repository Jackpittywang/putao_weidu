...
//                log("onPageSelected:" + position);
                if (position < FakePositionHelper.getStartPosition(AutoScrollViewPager.this) || position > FakePositionHelper.getEndPosition(AutoScrollViewPager.this)) {
                    log("position:" + position + "->" + FakePositionHelper.getRealPositon(AutoScrollViewPager.this, position) + "-return");
...
//                log("onPageScrollStateChanged->" + sta);
            ...
            public void dispatchMessage(Message msg) {
                switch (msg.what) {
                    case MSG_AUTO_SCROLL:
                        ...
                        break;
                    case MSG_SET_PAGE:
                        setFakeCurrentItem(FakePositionHelper.getRealPositon(AutoScrollViewPager.this, msg.arg1), false);
                        break;
                }
            }
        };
    ...
    public void setAdapter(PagerAdapter adapter) {
        ...
        mIsInfinitePagerAdapter = getAdapter() instanceof AutoScrollPagerAdapter;
        if (!mIsInfinitePagerAdapter) {
            throw new IllegalArgumentException("Currently, only InfinitePagerAdapter is supported");
        }
        setFakeCurrentItem(FakePositionHelper.getRealPositon(AutoScrollViewPager.this, 0), false);
    }
    ...
    public static class FakePositionHelper {
        ...
        public static int getRealFromFake(AutoScrollViewPager viewPager, int fake) {
        ...
        public static int getFakeFromReal(AutoScrollViewPager viewPager, int real) {
            int realAdapterSize = viewPager.getAdapterSize() / MULTIPLIER;
            if (realAdapterSize == 0)
                return 0;
        ...
        public static int getStartPosition(AutoScrollViewPager viewPager) {
            int realAdapterSize = viewPager.getAdapterSize() / MULTIPLIER;
            return realAdapterSize;
        }

        public static int getEndPosition(AutoScrollViewPager viewPager) {
            int realAdapterSize = viewPager.getAdapterSize() / MULTIPLIER;
            return realAdapterSize * (MULTIPLIER - 1) - 1;
        }

        public static int getRealAdapterSize(AutoScrollViewPager viewPager) {
        ...
        public static int getAdapterSize(ViewPager viewPager) {
            if (viewPager instanceof AutoScrollViewPager) {
                AutoScrollViewPager infiniteViewPager = (AutoScrollViewPager) viewPager;
                return getRealAdapterSize(infiniteViewPager);
            }
            PagerAdapter adapter = viewPager.getAdapter();
            if (adapter instanceof AutoScrollPagerAdapter) {
                AutoScrollPagerAdapter autoScrollPagerAdapter = (AutoScrollPagerAdapter) viewPager.getAdapter();
                return autoScrollPagerAdapter.getItemCount();
            }
            return adapter == null ? 0 : adapter.getCount();
        }

        public static int getRealPositon(AutoScrollViewPager viewPager, int position) {
