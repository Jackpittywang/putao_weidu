...
import com.putao.wd.pt_me.address.fragment.DistrictFragment;
...
public class AddressEditActivity extends PTWDActivity<GlobalApplication> implements View.OnClickListener, TextWatcher, SwitchButton.OnSwitchClickListener {
    ...
    private void addressUpdate(String address_id, String realname, String city_id, String province_id, String area_id, String address, String mobile, String tel, String postcode, String status) {
        if (isAddressEmpty) {
            status = "1";
            YouMengHelper.onEvent(mContext, YouMengHelper.UserHome_address_edit_default);
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.ll_delete_address://删除本条地址
                YouMengHelper.onEvent(mContext, YouMengHelper.UserHome_address_edit_delet);
...
//            ToastUtils.showToastShort(mContext, "您还有地址信息没有填写哟!");
        ...
        YouMengHelper.onEvent(mContext, YouMengHelper.UserHome_address_edit_save);
        finish();
    }
    ...
    public void onLeftAction() {
        super.onLeftAction();
        YouMengHelper.onEvent(mContext, YouMengHelper.UserHome_address_edit_cancel);
    }
